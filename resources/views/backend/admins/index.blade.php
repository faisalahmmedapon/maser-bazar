...
@section('content')
    <div class="d-flex justify-content-between">
        ...
        <span class="badge bg-info"><a class="text-white" href="{{ route('admins.create') }}"> New Admin </a></span>
    </div>
                ...
                <table id="example2" class="table table-striped table-bordered">
                    ...
                    @foreach ($admins as $key => $admin)
                        <tr>
                            ...
                            <td class="d-flex">
                                @can('admin-list')
                                    <a href="{{ route('admins.edit', $admin->id) }}" class="btn btn-sm"><i class="bx bxs-edit"></i></a>
                                @endcan
                                @can('admin-edit')
                                ...
                                @can('admin-delete')
                                    <form method="POST" action="{{ route('admins.destroy', $admin->id) }}"
                                        ...
                                        @csrf
                                        @method('DELETE')
                                        <button class="btn btn-sm" type="submit"><i class="bx bxs-trash"></i></button>
                                    </form>
                                @endcan
                            </td>
                        </tr>
